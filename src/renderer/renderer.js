...
const contenedorEstado = document.getElementById('estado-consulta');
const contenedorGraficos = document.getElementById('contenedor-graficos');
const contenedorFiltroAccion = document.getElementById('contenedor-filtro-accion');
const selectorAccion = document.getElementById('filtro-accion');

const lienzoGraficoDuracion = document.getElementById('grafico-duracion');
const lienzoGraficoCantidad = document.getElementById('grafico-cantidad');

let graficoDuracionPromedio;
let graficoCantidadPorMinuto;
 ...
 */
function mostrarEstadoDeConsulta(mensaje, tipo = 'info') {
 ...
 */
function bloquearUIDuranteCarga(estaCargando) {
 ...
 */
function normalizarRegistroCrudo(registroCrudo) {
  return {
 ...
 */
function agruparRegistrosPorMinuto(registros) {
 ...
 */
function convertirTiempoASegundos(valorTiempo) {
 ...
 */
function calcularMetricasParaGraficos(registrosFiltrados) {
  const agrupados = agruparRegistrosPorMinuto(registrosFiltrados);
  const etiquetas = [];
  const promediosPorMinuto = [];
  const conteosPorMinuto = [];

  Array.from(agrupados.keys())
    .sort()
    .forEach((claveMinuto) => {
      const { tiemposSegundos, conteo } = agrupados.get(claveMinuto);
 ...
 */
function renderizarGraficoDeDuracion(etiquetas, promediosPorMinuto) {
 ...
 */
function renderizarGraficoDeConteo(etiquetas, conteosPorMinuto) {
 ...
 */
function actualizarGraficosConFiltroSeleccionado() {
...
async function manejarConsulta() {
  ...
  try {
    ...
    const accionesUnicas = Array.from(new Set(registrosNormalizados.map((registro) => registro.accion))).sort();
    ...
    mostrarEstadoDeConsulta(`Error al consultar: ${error.message}`, 'error');
    contenedorGraficos.hidden = true;
  } finally {
